...
- **Deploy to SitecoreAI.** Push content (items and media) from a Mockingbird workspace to a live SitecoreAI (XM Cloud) environment, preserving item GUIDs. Add a target environment via the Header (SitecoreAI Environments icon) with a CM host URL, Client ID, and Client secret from your SitecoreAI Deploy credentials; Test validates the connection. Deploy from the content tree (right-click > Packaging > Deploy to SitecoreAI) or from the package cart (Generate Package dialog > Deploy to SitecoreAI button). Choose a conflict strategy (Skip existing / Keep existing / Overwrite - the last is experimental pending live validation) and preview the plan (N create / N update / N skip, plus any blocking errors) before deploying. Progress streams live; cancel at any time. Environment definitions (name and host) are shared in `config.mockingbird`; credentials are stored locally per-developer in `config.mockingbird.environments.local` (gitignored).
